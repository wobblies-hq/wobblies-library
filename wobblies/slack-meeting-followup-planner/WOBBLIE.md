...
routines:
  ...
  - Reply in the same Slack thread with a maximum of five concrete next steps Wobbly could help execute.
deny:
...
The watch conditions are intended for Slack messages or thread replies that already appear to contain meeting notes, a meeting transcript, or a link or file that Wobbly can read as meeting notes.
...
No-op when the content is clearly private, unrelated to the mapped repository or team, too thin to extract actions, mostly social, mostly status-only, or only a recording link that Wobbly cannot access.

## Reading policy

Use the Slack message, thread context, linked transcript, and attached note files as source material. When a transcript link or file is unavailable, no-op rather than asking for access unless the thread explicitly asks Wobbly to help.
...
3. Repository context needed to make a suggested Wobbly task concrete.

Do not invent owners, deadlines, or commitments. Mark uncertainty explicitly.

## Slack reply format

Reply in the same Slack thread using Slack `mrkdwn`. Keep the whole reply short and include a maximum of five concrete next steps Wobbly could help execute. Each next step must be 1-2 sentences long.
...
This wobbly plans follow-up only. It must not create issues, edit pull requests, assign owners, message people, or update trackers automatically.

Each item under `Wobbly can help` should be phrased as a task a human could ask Wobbly to perform next. Omit items that only record an existing tracked issue or require a human decision before Wobbly could reasonably act.

## Idempotency

Before replying, inspect the thread for an existing Wobbly meeting-follow-up reply for the same notes or transcript link. If one exists and the source notes are unchanged, no-op.
...
- there are no concrete follow-up options Wobbly could help execute
- an equivalent Wobbly follow-up plan already exists
