...
describe('wobblie install PR API', () => {
  test('creates a deterministic branch, atomic commit, educational PR body, and hidden marker without raw adaptation values', async () => {
    ...
    expect(result.filesPlanned.map((file) => file.destinationPath)).toEqual([
      '.wobblies/templated-wobblie/WOBBLIE.md',
      '.wobblies/templated-wobblie/scripts/render.sh',
      '.wobblies/templated-wobblie/references/render.md',
    ]);
    ...
    expect(createTreeCall?.options).toMatchObject({
      body: {
        ...
        tree: expect.arrayContaining([
          expect.objectContaining({ path: '.wobblies/templated-wobblie/WOBBLIE.md', mode: '100644', type: 'blob' }),
          expect.objectContaining({ path: '.wobblies/templated-wobblie/scripts/render.sh', mode: '100755', type: 'blob' }),
        ]),
      },
    });
...
This PR installs the \`templated-wobblie\` Wobblie wobblie to \`.wobblies/templated-wobblie/WOBBLIE.md\`. It was generated by Wobblie from the [\`templated-wobblie\` example](https://github.com/wobblie-hq/wobblies-library/blob/master/wobblies/templated-wobblie/WOBBLIE.md).
...
This PR adds that role at \`.wobblies/templated-wobblie/WOBBLIE.md\`. The file controls:
...
${result.markerText}`);
      ...
      async request<T>(method: string, requestPath: string, options?: unknown): Promise<T> {
        ...
        if (method === 'GET' && requestPath.endsWith('/git/trees/existing-tree')) {
          return {
            sha: 'existing-tree',
            tree: [
              { path: '.wobblies/templated-wobblie/WOBBLIE.md', type: 'blob', mode: '100644', sha: gitBlobSha(renderedWobblie) },
              { path: '.wobblies/templated-wobblie/scripts/render.sh', type: 'blob', mode: '100755', sha: gitBlobSha(renderedScript) },
              { path: '.wobblies/templated-wobblie/references/render.md', type: 'blob', mode: '100644', sha: gitBlobSha(renderedReference) },
            ],
          } as T;
        }
        if (method === 'POST' && requestPath.endsWith('/pulls')) {
    ...
    const githubClient: WobblieInstallPrGitHubClient = {
      async request<T>(method: string, requestPath: string, options?: unknown): Promise<T> {
        ...
        if (method === 'GET' && requestPath.endsWith('/git/trees/base-tree')) {
          return {
            sha: 'base-tree',
            tree: [{ path: '.wobblies/templated-wobblie/WOBBLIE.md', type: 'blob', mode: '100644', sha: 'old' }],
          } as T;
        }
        throw new Error(`unexpected GitHub request ${method} ${requestPath}`);
      },
  ...
  test('lists marker-backed PRs and reconciles edited-body install branches', async () => {
    ...
    const marker = createWobblieInstallMarker({
      ...
      targetDirectory: '.wobblies/templated-wobblie',
      files: ['.wobblies/templated-wobblie/WOBBLIE.md'],
